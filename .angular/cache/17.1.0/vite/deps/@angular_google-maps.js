    ...
    if (this._map._isBrowser) {
      if (google.maps.BicyclingLayer && this._map.googleMap) {
        ...
        this._zone.runOutsideAngular(() => {
          Promise.all([this._map._resolveMap(), importLibrary("maps", "BicyclingLayer")]).then(([map2, layerConstructor]) => {
            this._initialize(map2, layerConstructor);
          });
        });
      }
    }
  ...
  ngOnInit() {
    ...
    this._combineOptions().pipe(take(1)).subscribe((options) => {
      if (google.maps.Circle && this._map.googleMap) {
        ...
        this._ngZone.runOutsideAngular(() => {
          Promise.all([this._map._resolveMap(), importLibrary("maps", "Circle")]).then(([map2, circleConstructor]) => {
            this._initialize(map2, circleConstructor, options);
          });
        });
      }
    });
  ...
  ngOnInit() {
    if (this._googleMap._isBrowser) {
      if (google.maps.DirectionsRenderer && this._googleMap.googleMap) {
        ...
        this._ngZone.runOutsideAngular(() => {
          Promise.all([this._googleMap._resolveMap(), importLibrary("routes", "DirectionsRenderer")]).then(([map2, rendererConstructor]) => {
            this._initialize(map2, rendererConstructor);
          });
        });
      }
    }
    ...
    if (this._map._isBrowser) {
      this._bounds.pipe(takeUntil(this._destroyed)).subscribe((bounds) => {
        ...
        if (google.maps.GroundOverlay && this._map.googleMap) {
          ...
          this._ngZone.runOutsideAngular(() => {
            Promise.all([this._map._resolveMap(), importLibrary("maps", "GroundOverlay")]).then(([map2, overlayConstructor]) => {
              this._initialize(map2, overlayConstructor, bounds);
            });
          });
        }
      });
    ...
    if (this._map._isBrowser) {
      this._combineOptions().pipe(take(1)).subscribe((options) => {
        if (google.maps.KmlLayer && this._map.googleMap) {
          ...
          this._ngZone.runOutsideAngular(() => {
            Promise.all([this._map._resolveMap(), importLibrary("maps", "KmlLayer")]).then(([map2, layerConstructor]) => {
              this._initialize(map2, layerConstructor, options);
            });
          });
        }
      });
...
var _MapMarker = class _MapMarker {
  ...
  ngOnInit() {
    ...
    if (google.maps.Marker && this._googleMap.googleMap) {
      ...
      this._ngZone.runOutsideAngular(() => {
        Promise.all([this._googleMap._resolveMap(), importLibrary("marker", "Marker")]).then(([map2, markerConstrutor]) => {
          this._initialize(map2, markerConstrutor);
        });
      });
    }
  }
    ...
    if (this._map._isBrowser) {
      this._combineOptions().pipe(take(1)).subscribe((options) => {
        if (google.maps.Polygon && this._map.googleMap) {
          ...
          this._ngZone.runOutsideAngular(() => {
            Promise.all([this._map._resolveMap(), importLibrary("maps", "Polygon")]).then(([map2, polygonConstructor]) => {
              this._initialize(map2, polygonConstructor, options);
            });
          });
        }
      });
    ...
    if (this._map._isBrowser) {
      this._combineOptions().pipe(take(1)).subscribe((options) => {
        if (google.maps.Polyline && this._map.googleMap) {
          ...
          this._ngZone.runOutsideAngular(() => {
            Promise.all([this._map._resolveMap(), importLibrary("maps", "Polyline")]).then(([map2, polylineConstructor]) => {
              this._initialize(map2, polylineConstructor, options);
            });
          });
        }
      });
    ...
    if (this._map._isBrowser) {
      this._combineOptions().pipe(take(1)).subscribe((options) => {
        if (google.maps.Rectangle && this._map.googleMap) {
          ...
          this._ngZone.runOutsideAngular(() => {
            Promise.all([this._map._resolveMap(), importLibrary("maps", "Rectangle")]).then(([map2, rectangleConstructor]) => {
              this._initialize(map2, rectangleConstructor, options);
            });
          });
        }
      });
    ...
    if (this._map._isBrowser) {
      this._combineOptions().pipe(take(1)).subscribe((options) => {
        if (google.maps.TrafficLayer && this._map.googleMap) {
          ...
          this._ngZone.runOutsideAngular(() => {
            Promise.all([this._map._resolveMap(), importLibrary("maps", "TrafficLayer")]).then(([map2, layerConstructor]) => {
              this._initialize(map2, layerConstructor, options);
            });
          });
        }
  ...
  ngOnInit() {
    if (this._map._isBrowser) {
      if (google.maps.TransitLayer && this._map.googleMap) {
        ...
        this._zone.runOutsideAngular(() => {
          Promise.all([this._map._resolveMap(), importLibrary("maps", "TransitLayer")]).then(([map2, layerConstructor]) => {
            this._initialize(map2, layerConstructor);
          });
        });
      }
  ...
  ngOnInit() {
    if (this._googleMap._isBrowser) {
      ...
      if (google.maps.visualization?.HeatmapLayer && this._googleMap.googleMap) {
        ...
        this._ngZone.runOutsideAngular(() => {
          Promise.all([this._googleMap._resolveMap(), importLibrary("visualization", "HeatmapLayer")]).then(([map2, heatmapConstructor]) => {
            this._initialize(map2, heatmapConstructor);
          });
        });
      }
    }
